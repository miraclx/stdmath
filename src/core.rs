...
#[macro_export]
macro_rules! infix {
    // credits: durka42 (https://stackoverflow.com/a/36800597/9806233)

    // done converting
    (@cvt () $postfix:tt) => { $crate::infix!(@pfx () $postfix) };
    //                                        |    |  ^ postfix expression
    //                                        |    ^ operand stack
    //                                        ^ postfix interpreter
    ...
    (@cvt ($ophead:tt $($optail:tt)*) ($($postfix:tt)*)) => { $crate::infix!(@cvt ($($optail)*) ($($postfix)* $ophead)) };

    // 2. push an operator onto the stack if it's empty or has a left-paren on top
    (@cvt (                 ) $postfix:tt + $($tail:tt)*) => { $crate::infix!(@cvt (+               ) $postfix $($tail)*) };
    (@cvt (                 ) $postfix:tt - $($tail:tt)*) => { $crate::infix!(@cvt (-               ) $postfix $($tail)*) };
    (@cvt (                 ) $postfix:tt * $($tail:tt)*) => { $crate::infix!(@cvt (*               ) $postfix $($tail)*) };
    (@cvt (                 ) $postfix:tt / $($tail:tt)*) => { $crate::infix!(@cvt (/               ) $postfix $($tail)*) };
    (@cvt (LP $($optail:tt)*) $postfix:tt + $($tail:tt)*) => { $crate::infix!(@cvt (+ LP $($optail)*) $postfix $($tail)*) };
    (@cvt (LP $($optail:tt)*) $postfix:tt - $($tail:tt)*) => { $crate::infix!(@cvt (- LP $($optail)*) $postfix $($tail)*) };
    (@cvt (LP $($optail:tt)*) $postfix:tt * $($tail:tt)*) => { $crate::infix!(@cvt (* LP $($optail)*) $postfix $($tail)*) };
    (@cvt (LP $($optail:tt)*) $postfix:tt / $($tail:tt)*) => { $crate::infix!(@cvt (/ LP $($optail)*) $postfix $($tail)*) };

    // 3. push a left-paren onto the stack
    (@cvt ($($operator:tt)*) $postfix:tt ($($inner:tt)*) $($tail:tt)*) => { $crate::infix!(@cvt (LP $($operator)*) $postfix $($inner)* RP $($tail)*) };

    // 4. see right-paren, pop operators to postfix until left-paren
    (@cvt (LP         $($optail:tt)*) $postfix:tt       RP $($tail:tt)*) => { $crate::infix!(@cvt ($($optail)*) $postfix               $($tail)*)    };
    (@cvt ($ophead:tt $($optail:tt)*) ($($postfix:tt)*) RP $($tail:tt)*) => { $crate::infix!(@cvt ($($optail)*) ($($postfix)* $ophead) RP $($tail)*) };

    // 5. if an operator w/ lower precedence is on top, just push
    (@cvt (+ $($optail:tt)*) $postfix:tt * $($tail:tt)*) => { $crate::infix!(@cvt (* + $($optail)*) $postfix $($tail)*) };
    (@cvt (- $($optail:tt)*) $postfix:tt * $($tail:tt)*) => { $crate::infix!(@cvt (* - $($optail)*) $postfix $($tail)*) };
    (@cvt (+ $($optail:tt)*) $postfix:tt / $($tail:tt)*) => { $crate::infix!(@cvt (/ + $($optail)*) $postfix $($tail)*) };
    (@cvt (- $($optail:tt)*) $postfix:tt / $($tail:tt)*) => { $crate::infix!(@cvt (/ - $($optail)*) $postfix $($tail)*) };
    ...
    (@cvt $operators:tt ($($postfix:tt)*) $head:tt $($tail:tt)*) => { $crate::infix!(@cvt $operators ($($postfix)* ($head)) $($tail)*) };

    // postfix interpreter
    (@pfx ($result:expr                     ) (                     )) => { $result };
    (@pfx (($a:expr) ($b:expr) $($stack:tt)*) (+        $($tail:tt)*)) => { $crate::infix!(@pfx ((($crate::Resolve::to_context($b) + $crate::Resolve::to_context($a))) $($stack)*) ($($tail)*)) };
    (@pfx (($a:expr) ($b:expr) $($stack:tt)*) (-        $($tail:tt)*)) => { $crate::infix!(@pfx ((($crate::Resolve::to_context($b) - $crate::Resolve::to_context($a))) $($stack)*) ($($tail)*)) };
    (@pfx (($a:expr) ($b:expr) $($stack:tt)*) (*        $($tail:tt)*)) => { $crate::infix!(@pfx ((($crate::Resolve::to_context($b) * $crate::Resolve::to_context($a))) $($stack)*) ($($tail)*)) };
    (@pfx (($a:expr) ($b:expr) $($stack:tt)*) (/        $($tail:tt)*)) => { $crate::infix!(@pfx ((($crate::Resolve::to_context($b) / $crate::Resolve::to_context($a))) $($stack)*) ($($tail)*)) };
    (@pfx ($($stack:tt)*                    ) ($head:tt $($tail:tt)*)) => { $crate::infix!(@pfx ($head       $($stack)*) ($($tail)*)) };

    ($($t:tt)*) => { $crate::infix!(@cvt () () $($t)*) }
    //                              |    |  |  ^ infix expression
    //                              |    |  ^ postfix expression
    //                              |    ^ operator stack
