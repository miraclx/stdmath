...
impl<T, R, F: Fn(T) -> R> std::ops::Mul<Context7<T, R, F>> for Context7<T, R, F>
where
    T: Clone + 'static,
    R: One
        + Zero
...
{
    type Output = Context7<R, R, fn(R) -> R>;
    fn mul(self, rhs: Context7<T, R, F>) -> Self::Output {
        product7(
...
impl<T, R, F: Fn(T) -> R> std::ops::Sub<Context7<T, R, F>> for Context7<T, R, F>
where
    T: Clone + 'static,
    R: One
        + Zero
...
{
    type Output = Context7<R, R, fn(R) -> R>;
    fn sub(self, rhs: Context7<T, R, F>) -> Self::Output {
        sum7(
            vec![Type::Normal(self), Type::Inverse(rhs)].into_iter(),
            |x| x,
        )
